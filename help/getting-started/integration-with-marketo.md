---
title: 与Marketo集成
description: 了解Brand Concierge如何捕获潜在客户和书籍会议，将每次访客互动作为Smart Campaigns的原生活动数据流入Marketo。
role: Admin
level: Beginner
duration: 215
topic: Integrations
doc-type: Tutorial
last-substantial-update: 2026-06-10T00:00:00Z
jira: KT-21468
source-git-commit: e77153743febdcd411ce973172c82a7a713b4135
workflow-type: tm+mt
source-wordcount: '153'
ht-degree: 0%

---

# 与Marketo集成

了解Brand Concierge如何关闭您的网站与Marketo Engage之间的循环。 了解访客对话（从聊天中潜在客户捕获到会议预订）如何作为本机活动数据自动流入Marketo，以及如何将该数据用于触发和批量智能营销活动。

## 此视频面向谁？

* 管理员

## 视频内容

* 访客对话如何从Brand Concierge流入Marketo — 从聊天中商机捕获和会议预订到自动商机创建
* 潜在客户的活动日志中记录的本机“在礼宾室中计划的会议”活动类型，具有完整上下文：会议持续时间、销售步骤、路由类型和预订状态
* 如何在Marketo Smart Campaigns中使用Brand Concierge活动类型 — 作为触发器、过滤器或批量活动数据

>[!VIDEO](https://video.tv.adobe.com/v/3492225?learn=on)

有关文档，请参阅[Brand Concierge帮助](../documentation/overview.md)。
