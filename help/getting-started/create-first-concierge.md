---
title: 创建礼宾 — 视频
description: 了解如何设置Adobe Brand Concierge，以使用您品牌的声音和准则在您的网站上提供个性化、对话式的客户体验。
topic: Personalization,Integrations
role: User
level: Beginner
doc-type: Tutorial
duration: 372
last-substantial-update: 2026-08-20
jira: KT-19504
TQID: https://experienceleague.adobe.com/RDjW-BGiusLzV4wcUQpJ7kB6VxE01H-a6iaDlwbGofc
product_v2:
  - id: b6ee73fe-bdc6-47d9-99a2-80194514dd40
role_v2:
  - id: b69b2659-1057-424e-8fc5-ed9e016dc554
level_v2:
  - id: e8ccd51f-da0d-4e3b-939b-e30d5ebb1ea5
topic_v2:
  - id: a09a5a04-e30b-4d55-b031-38e6f5ec86db
  - id: bce87dde-a4ab-44c9-8a18-ad66e4ddb377
  - id: e0eb8757-182f-49f3-94a4-1587d16f5094
source-git-commit: 5e6249cc834071ae1f330278a4150fe6140da758
workflow-type: tm+mt
source-wordcount: 160
ht-degree: 0%

---

# 创建您的第一个礼宾

了解如何在几分钟内设置您的第一个Adobe Brand Concierge。 提供您的网站URL，Brand Concierge会自动根据您网站的内容构建入门品牌配置文件和说明，您可以在保存之前查看和调整这些配置文件和说明。 构建礼宾后，在桌面和移动设备上使用示例问题对其进行测试，以了解它如何使用您自己的网站内容做出响应。

## 此视频面向谁？

* Digital Experience Manager
* 营销技术人员
* 客户体验Designer

## 视频内容

* 如何输入您的网站URL以自动填充您的品牌配置文件和建议说明
* 如何在保存之前查看和调整品牌配置文件、说明以及入门提示
* 如何在桌面和移动设备上通过示例问题测试您的礼宾

>[!VIDEO](https://video.tv.adobe.com/v/3476531?captions=chi_hans&learn=on)

有关文档，请参阅[Brand Concierge帮助](../documentation/overview.md)。

