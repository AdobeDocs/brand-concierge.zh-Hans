---
title: 准备内容和访客历程 — 视频
description: 了解如何准备知识源并定义访客历程，以形成Adobe Brand Concierge回答问题和指导访客的方式。
topic: Personalization,Integrations
role: User
level: Beginner
doc-type: Tutorial
duration: 198
last-substantial-update: 2026-07-17T00:00:00Z
jira: KT-21785
source-git-commit: 42d92594723820d8589e07144e39a9a8a911c95b
workflow-type: tm+mt
source-wordcount: '150'
ht-degree: 0%

---


# 准备内容和访客历程

Adobe Brand Concierge只能回答您的问题，以及您为其提供的内容。 在网站投入使用之前，您需要准备以下两件事：从中获取的知识源，以及要投放的访客体验的简短定义。

## 此视频面向谁？

* 内容和网站所有者
* 准备启动礼宾的营销团队
* 任何为Brand Concierge收集原始资料的人员

## 视频内容

* 三种类型的知识源：网站URL、PDF和产品电子表格
* 如何在共享内容之前准备内容
* 如何编写单页访客历程定义

>[!VIDEO](https://video.tv.adobe.com/v/3496007/?learn=on)

## 本系列中的相关视频

* [欢迎使用上线历程](welcome.md)

## 文档

有关文档，请参阅[Brand Concierge帮助](../documentation/overview.md)。
